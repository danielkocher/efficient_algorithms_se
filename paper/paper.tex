...
upon Paxos~\cite{Lamport:1998} state machines on machines spread across multiple
datacenters across the world. This global replication guarantees high availability
and also implies some desirable locality properties which benefit the performance.
...
to deal with complex and evolving schemas. Moreover, Bigtable only guarantees to
be eventually-consistent. In contrast, Spanner provides much stronger consistency
guarantees. On the other hand, Megastore suffers from poor write throughput even
though it is used for many applications.

Therefore, Google decided to introduce another distributed, highly available
database system which evolved from a key-value store somehow similar to the one
used by Bigtable (but we will see that this is only partially correct). A core
feature provided by Spanner are so-called \emph{externally-consistent} distributed
general-purpose transactions. Of course, Spanner also provides an SQL-like query
language to enable applications/users to interact with Spanner in a well-known
way. The data is stored in schematized \emph{semi-relational} tables and the data
is versioned automatically. Google also implemented a garbage collection to deal
with old data. The replication can be dynamically configured in order to control
the locality of the replicas. This is useful to control write and read latency
as well as availability. Another main contribution of Spanner are
\emph{globally-consistent} reads at a specific timestamp. This and
externally-consistent reads and writes enable Spanner to provide a special type
of transaction, so-called \emph{schema-change} transactions, which provides atomic
schema changes without locking the whole database and all ongoing transactions.
...
which, essentially, is a debugging console and shows the status of each zone. A
...
continents. Using such a physical isolation, Spanner is resistent to events which
affect large areas, e.g. natural disasters or war. Hence, a well-configured
